...
import { EDITOR_FONT, getPageContentDimensions } from '@/lib/editor/constants'
import { Cursor } from './Cursor'
import { useEditorContext } from './EditorContext'
import { parseMarkdown, renderFormattedNodes } from '@/lib/editor/markdown'
import { measureText, getOffsetAtX } from '@/lib/editor/textMeasurement'
...
export function PageSegment({ segment }: PageSegmentProps) {
  ...
  const handleClick = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      ...
      // Get the appropriate font size for this segment type
      let fontSize: number = EDITOR_FONT.size
      switch (segment.contentType) {
        case 'title':
          fontSize = EDITOR_FONT.mainTitleSize
          break
        case 'author':
          fontSize = EDITOR_FONT.authorSize
          break
        case 'chapter-title':
          fontSize = EDITOR_FONT.titleSize
          break
      }

      const lineHeight = fontSize * EDITOR_FONT.lineHeight
      const { width: maxWidth } = getPageContentDimensions()

      // Get line measurement for the segment text
      const measurement = measureText(segment.text, maxWidth, fontSize)

      // Determine which line was clicked
      const lineIndex = Math.min(Math.floor(y / lineHeight), measurement.lineCount - 1)

      // Find the start and end offsets of the clicked line
      let lineStart = 0
      let lineEnd = segment.text.length

      if (lineIndex > 0 && measurement.lineBreaks.length > 0) {
        lineStart = measurement.lineBreaks[lineIndex - 1]
        // Skip newline/space at the start of line
        if (segment.text[lineStart] === '\n' || segment.text[lineStart] === ' ') {
          lineStart++
        }
      }

      if (lineIndex < measurement.lineBreaks.length) {
        lineEnd = measurement.lineBreaks[lineIndex]
      }

      // Get the exact character offset at the x position
      const offset = getOffsetAtX(segment.text, lineStart, lineEnd, x, fontSize)
